...
class GameScreen(private val mGame:JumpActionGame): ScreenAdapter() {
    companion object {
        ...
        val WORLD_WIDTH = 10f
        val WORLD_HEIGHT = 15 * 20 //20画面分のぼれば終了
        val GUI_WIDTH = 320f
        val GUI_HEIGHT = 480f
    ...
    init {
        ...
        mSound1=Gdx.audio.newSound(Gdx.files.internal("data/mysound.mp3"))
        //背景の準備
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.masaaki.kabe.jumpactiongame")
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            if (y>2){
                if (mRandom.nextFloat() > 0.8f) {
                    ...
                    enemy.setPosition(step.x + mRandom.nextFloat()*2, step.y + Enemy.Enemy_HEIGHT + mRandom.nextFloat() * 3)
                    mEnemy.add(enemy)
                }
            }
    ...
    private fun checkGameOver() {
        if (mHeightSoFar - CAMERA_HEIGHT / 2 > mPlayer.y) {
            Gdx.app.log("JampActionAame", "GAMEOVER")
            mGameState = GAME_STATE_GAMEOVER
        }
    }
