...
class Player (texture: Texture, srcX:Int, srcY:Int,srcWidth:Int,srcHeight:Int):GameObject(texture,srcX,srcY,srcWidth,srcHeight){
    ...
    fun update(delta:Float,accelX:Float){
        ...
        //y方向の速度が正（＝上方）のときにSTATEがPLAYER_STATE_JUMPでなければ、PLAYER_STATE_JUMPにする
        if(velocity.y>0){
        ...
        //y方向の速度が負（＝下方向）のときにSTATEがPLAYER_STATE_JUMPでなければ、PLAYER_STATE_JUMPにする
        if(velocity.y<0){
